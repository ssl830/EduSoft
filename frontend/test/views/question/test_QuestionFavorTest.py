...
BASE_URL = "http://localhost:3000/questionFavor"
...
def test_question_favor_render(playwright, credentials, role):
    ...
    if role == "admin":
        ...
        browser.close()
        return
    expect(page).to_have_url(re.compile(r"http://localhost:3000/questionFavor"))
    expect(page.locator("h2")).to_have_text("收藏题库")
    # 检查表格渲染或空提示
    if page.locator(".resource-table").count() > 0 and page.locator(".resource-table").is_visible():
        ...
        # 检查取消收藏按钮
        unfavor_btns = page.locator("button.download")
        if unfavor_btns.count() > 0:
            unfavor_btns.first.click()
            # 取消收藏后表格刷新，断言页面未报错
