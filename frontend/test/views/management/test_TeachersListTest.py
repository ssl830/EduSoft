...
BASE_URL = "http://localhost:3000/manage/teachers"
...
def test_teachers_list_render(playwright):
    browser, context, page = login_and_get_page(playwright, TUTOR_CREDENTIALS, BASE_URL)
    expect(page.locator("h2")).to_have_text("教师基本信息列表")
    expect(page.locator(".teachers-table")).to_be_visible(timeout=10000)
    # 检查表头字段
    headers = ["ID", "用户ID", "姓名", "邮箱", "创建时间", "操作"]
    for h in headers:
        expect(page.locator(f".teachers-table th:text-is('{h}')")).to_be_visible()
    # 检查至少有一行数据
    row_count = page.locator(".teachers-table tbody tr").count()
    assert row_count > 0
    # 检查删除按钮可见
...
def test_teachers_list_delete_modal(playwright):
    browser, context, page = login_and_get_page(playwright, TUTOR_CREDENTIALS, BASE_URL)
    # 点击第一个删除按钮
    ...
    expect(page.locator(".modal-content")).to_have_text("确定要删除该教师吗？")
    # 取消按钮关闭弹窗
