...
public class CourseClient extends BaseServiceClient {
    ...
    public Map<String, Object> getSectionById(Long sectionId) {
        ...
        String token = getCurrentToken();
        logger.info("[CourseClient] 获取章节信息，章节ID: {}, token: {}", sectionId, token);
        ...
        org.springframework.http.HttpEntity<Void> entity = new org.springframework.http.HttpEntity<>(headers);
        String url = getBaseUrl() + "/api/course-sections/section/" + sectionId;
        ...
        try {
            ...
            logger.info("[CourseClient] 获取章节信息成功，返回数据: {}", result);
            return result;
        } catch (Exception e) {
            logger.error("[CourseClient] 获取章节信息失败: {}", e.getMessage());
            throw e;
        }
    }
